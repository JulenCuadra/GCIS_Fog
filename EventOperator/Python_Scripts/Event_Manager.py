...
action_list = {'dummy_action', 'not_dummy_action'}
...
def handle_event(event, action):
    ...
    if action_is_available(action.name):
        ...
        #TODO habría que añadir los parámetros de customization.

    #Todo, borrar cuando funcione lo anterior.
    match event.action:
        case "TEST":
            job = tipos.job_event_handler("test-name" + ''.join(random.choices(string.ascii_lowercase + string.digits, k=5)))
            job['spec']['template']['spec']['containers'][0]['image'] = "perl:5.34.0"
            job['spec']['template']['spec']['containers'][0]['command']= ["perl", "-Mbignum=bpi", "-wle", "print bpi(2000)"]
            client_batch.create_namespaced_job(namespace, job)


def check_event_action_relationships(source,event):
    #TODO checkear si es compatible con la última version de definición de Component.
    resource = client_customresources.get_namespaced_custom_object('misrecursos.aplicacion','v1alpha1','default','components')
    event_action_list = resource.spec.events
    if event.metadata.name in event_action_list: #Si el nombre del evento está entre el listado de eventos del recurso.
        for action in event_action_list.actions: #Por cada acción asociada a ese evento.
...
def watcher_events(client):
    ...
    for event in watcher.stream(client.list_event_for_all_namespaces):
        ...
        if 'cause' in objeto.metadata.labels:
            if objeto.metadata.labels.cause == 'gcis-ehu': #Si el evento ha sido lanzado por un recurso nuestro.
                check_event_action_relationships(objeto.source.component,objeto)
            else: # Cause existe pero no es de gcis-ehu, se presupone que no utilizan nuestra extensión.
                pass
