...
def event_parser(message):
    ...
    aux={
        'source' : {
        'component': message['CONTEXT']['source'], #TODO modificar source porque no acepta '/' compatibilizar eso con formato uri
    },
        'metadata':{
            'name' : message['CONTEXT']['source']+'-'+message['CONTEXT']['id']+'-event',
            'annotations':{
                'data' : string(message['DATA'])#Todo modificar data porque en las anotaciones solo acepta strings.
            },
            'labels': {
                'cause': 'ehu.gcis'
            }
...
def raise_application_event2k8s_event(message): #Debería discriminar de alguna forma el mensaje que le llega, que imagino será diferente desde MQTT o Kafka.
    ...
    #decodedMessage = loads(message.decode('utf-8'))
    ...
    #TODO gestionar como un try para que no colisione si el evento ya existe (a veces repite).
